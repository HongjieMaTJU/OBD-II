...
{
    OBDConfigure::instance()->wiringCheck();
}
...
{
    OBDConfigure::instance()->dumpBuffer();
}
...
{
    OBDConfigure::instance()->kwDisplay();
}
...
{
    OBDConfigure::instance()->closeProtocol();
}
...
static void OnSetProtocol(const string& cmd, int par)
{
    ...
    OBDConfigure::instance()->setProtocol(protocol, true);
    AdptSendReply(OkMessage);
}
...
{
    OBDConfigure::instance()->sendHeartBeat();
}
