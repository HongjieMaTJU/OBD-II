...
#include <driver_inc/UartLPC15xx.h>
#include <cstring>
#include <chip.h>
...
{
    // Enable UART0 clock
    LPC_SYSCON->SYSAHBCLKCTRL[1] |=  (1 << 17);
    LPC_SYSCON->PRESETCTRL[1]   |=  (1 << 17);
    LPC_SYSCON->PRESETCTRL[1]    &= ~(1 << 17);
   ...
   // GPIOPinConfig(TxPort, TxPin, 0); // open-drain
    ...
    LPC_SWM->PINASSIGN[0] &= 0xFFFF0000;
    LPC_SWM->PINASSIGN[0] |= PinAssign;
}
