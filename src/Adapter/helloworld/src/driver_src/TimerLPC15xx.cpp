...
{
    LPC_RITIMER ->CTRL = 0;
    LPC_RITIMER ->MASK = LPC_RITIMER ->MASK_H = 0;
...
void LongTimer::start(uint32_t interval)
{
    ...
    LPC_RITIMER ->CTRL = 0;
    LPC_RITIMER ->COUNTER = LPC_RITIMER ->COUNTER_H = 0;
    LPC_RITIMER ->COMPVAL = compval & 0xFFFFFFFF;
    LPC_RITIMER ->COMPVAL_H = compval >> 32;
    LPC_RITIMER ->CTRL = 0x09;
}
